...
let decode ~utf ~subject_literal ~(emit : string -> unit) (s : string)
  ...
  (try
     while not (!fatal || at !p = '\000') do
       ...
       if !c = Char.code ']' && !start_rep >= 0 then
           ...
           | Some (li, j) ->
               ...
               if at !p <> '}' then fail "** Expected '}' after \\[...]{..."
                 ...
                 if li <= 0 then fail "** Zero or negative repeat not allowed"
                   ...
                   let rep =
                     Buffer.sub q !start_rep (Buffer.length q - !start_rep)
                   in
                   for _k = 2 to li do
         ...
         (if !c <> Char.code '\\' || subject_literal then (
            if utf && !c >= 0xc0 then
            ...
            | '0' .. '7' ->
                ...
                let i = ref 0 in
                while
                  !i < 2 && Cstr.isdigit (at !p) && at !p <> '8' && at !p <> '9'
                do
            ...
            | 'o' ->
                if at !p = '{' then (
                  ...
                  let i = ref 0 in
                  while
                    Cstr.isdigit (at !pt) && at !pt <> '8' && at !pt <> '9'
                  do
                    incr i;
            ...
            | '[' ->
                if !start_rep >= 0 then (
                  ...
                  store := false)
            | ch ->
                if Cstr.isalnum ch then (
                  fail
                    (Printf.sprintf "** Unrecognized escape sequence \"\\%c\""
                       ch);
                  store := false));
         (* Store the code point (pcre2test.c:7169-7207, 8-bit arm) *)
         if !store && not !fatal then
           if utf then
