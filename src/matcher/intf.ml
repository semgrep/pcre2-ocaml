(* Shared pieces of the 7.5.3 convenience layer (quote, splitting results,
   substitution templates) and the [Matcher] signature, on top of the
   rewritten Result API. Moved verbatim from src/intf.ml in the Matcher-API
   factoring (fast-engine milestone chunk A) so both the pcre2 library and
   additional engines (pcre2.fast) share one copy; src/intf.ml now just
   includes this module. Ported from tag 7.5.3 src/pcre2.ml. *)
